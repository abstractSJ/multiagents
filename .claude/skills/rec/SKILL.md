...
- 主会话只负责：解析目标、检查有哪些产物、决定该委派给谁、整合最终结果。
...
- 目标：公司、代码、财年、报告类型。
- 已调用角色：`information-collector` / `information-processor` / `financial-analyst` / `valuation-analyst`。
- 复用产物：关键路径。
- 新生成产物：关键路径。
