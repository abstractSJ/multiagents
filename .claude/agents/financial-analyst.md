...
1. 先检查证据包完整性。
2. 再拆本期利润变化链，而不是先下结论。
3. 再验证利润质量、现金流质量和资产负债表支撑。
4. 再识别一次性项目、会计口径和附注风险。
5. 再形成规范化财务基数和未来假设边界。
6. 再形成预期差判断、风险和财务证伪条件。
7. 最后输出 `valuation_handoff`，把估值分析员需要的财务输入、调整项、折溢价理由和缺口讲清楚。
8. 证据不足时先回上游补证；若不阻塞财务判断，必须说明对估值输入的影响。
